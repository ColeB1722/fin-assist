...
**2026-05-16 (session 6 — PR #152 review triage):** Walked the 21 self-review comments on PR #152. Outcomes:

- **Inline fixes (this PR):** `AGENTS.md` doc section renamed Post-Merge → Pre-Merge Documentation Discipline (matches actual practice); `agents/mcp.py` transport imports hoisted to top-of-file and `_cm` typed as `AbstractAsyncContextManager[tuple[Any, Any]]`; `agents/skills.py` `SkillLoader.load_from_config` collapsed to `config.model_copy(update={"name": name})` with docstring spelling out that it's a name-stamp, not a transformation (artifact of #142 type collapse); test patch targets in `test_mcp.py` retargeted to `fin_assist.agents.mcp.stdio_client`/`sse_client` since imports are now hoisted. 972 tests passing.
- **Folded into existing issues:** comments on context-provider/built-in-tool overlap → #129; annotation→policy translation and policy enrichment → #139; sync→async bridge + context-manager lifecycle → #154.
- **New issues filed:** #155 (built-in tool schemas as pydantic models, consistent across all 5); #156 (per-subcommand approval at executor level — was unfiled v0.1.1 scope item referenced in code comments); #158 (MCP `CallToolResult.isError` + `structuredContent` handling gaps).
- **Triage lesson captured:** filed #157 ("MCP SDK ergonomics audit") deferring a "do the helpers we hand-rolled already exist upstream?" question to "next major bump" — then realized the verification cost was 5 minutes and the deferral was busywork. Did the audit, closed #157, filed #158 for the *real* bugs the audit surfaced (`isError` ignored, `structuredContent` ignored). **Going forward: when a self-review comment is shaped as "is X true?" / "shouldn't Y exist?", verify before filing.** Deferred-audit issues without doing the audit are a category of busywork to avoid.

**2026-05-17 (session 7 — CI required-checks deadlock):** PR #152 merged before the handoff doc-update commit (`a925b5d`) was pushed.  Recovery PR #159 opened with just the doc fix, then hit the "[required status check deadlock](docs/decisions.md#ci-required-checks)" — the `main` ruleset required `format`/`lint`/`test` but `ci.yml`'s `paths-ignore: [docs/**, *.md]` meant doc-only PRs never reported those checks, leaving the PR perpetually pending.  Grounded the decision via web research (the duplicate-skip-workflow pattern GitHub used to recommend is now considered an anti-pattern; the community converged on "single aggregation job + skipped-as-success").  Reworked `.github/workflows/ci.yml` to drop `paths-ignore` entirely *and* add a `ci-required` sentinel job that aggregates all upstream job results.  Updated the `main` ruleset's required checks from `[format, lint, test]` to `[ci-required]`.  Documented the decision in `docs/decisions.md` § CI required checks.  Net effect: doc PRs run full CI (~6-9 min cached) but never deadlock, *and* path-conditional jobs can be reintroduced safely later without touching the ruleset.
