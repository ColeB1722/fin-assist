...
### Windows Dev Ergonomics (sketched 2026-05-12)

**Status:** Pre-v0.1.1 side quest. Implementation ready.

**Problem:** The only dev entry path requires Nix + devenv. On Windows (scoop + pwsh, no nix), `just ci` is broken — justfile uses bash-only syntax, `paths.py` defaults to `~/.local/share/fin/` (Unix convention), and there's no documented non-Nix setup. This means the project can't be demonstrated or developed on Windows.

**Research findings:** Scoop distributes pre-built binaries, not Python packages — it's the wrong tool for `fin-assist`. The Python-native equivalent is `uv tool install` (entry points already defined in `pyproject.toml`). Similar projects (uv, ruff, just) ship Scoop manifests for their compiled binaries; pure-Python tools (FastAPI, Textual, pydantic-ai) use pip/uv only. Cross-platform justfiles are supported natively via `set windows-shell`.

**Three layers, only two ship now:**

| Layer | Approach | Effort | Ship |
|-------|----------|--------|------|
| **Run/install fin** | `uv tool install -e .` | Zero — entry points already exist | Now |
| **Dev workflow** | Cross-platform justfile + `paths.py` fix | Low | Now |
| **Scoop manifest** | PyInstaller + Scoop JSON + CI pipeline | High | Deferred (pre-built binary story, revisit post-1.0) |

#### Changes

**1. `justfile` — cross-platform**
...
Recipes with Unix-isms get conditional branches:

| Recipe | Unix | Windows |
|--------|------|---------|
| `dev` | `devenv shell` | `uv sync --all-groups` |
| `fmt` | `treefmt` | `uv run ruff format src/` |
| `check` | `treefmt --ci` | `uv run ruff format --check src/` |
| `clean` | `rm -rf ...; find ... -exec rm -rf {} +` | `uv run python -c "import shutil, pathlib; ..."` |
| `diagrams-clean` | `rm -f docs/diagrams/*.svg ...` | `uv run python -c "..."` |

All other recipes (`lint`, `lint-fix`, `lint-imports`, `typecheck`, `test`, `test-cov`, `ci`, `run`, `install-dev`, `build`) already use `uv run` — no changes needed.

Note: `just` conditionals use `if os_family() == "windows"`. On Windows, just uses `windows-shell` automatically; elsewhere it uses `shell`. No recipe duplication needed.

**2. `paths.py` — platform-aware default**
...
- Windows: `%LOCALAPPDATA%/fin` (e.g. `C:\Users\cole\AppData\Local\fin`)
- Unix: `~/.local/share/fin/` (unchanged)
- `FIN_DATA_DIR` override still works everywhere
- `devenv.nix` sets `FIN_DATA_DIR=./.fin` so Nix users unaffected

**3. `README.md` — non-Nix setup path**

Add a "Quick start (no Nix)" section:
```
scoop install uv just     # or: brew install uv just
uv sync
just test
```

Document `uv tool install -e .` for running `fin` globally from a clone.

**4. CI — Windows job**

Add a `test-windows` job on `windows-latest` that runs `uv sync && just test` without Nix. This catches platform regressions in `paths.py`, file handling, and subprocess calls.

**5. Validation on Windows**

Manual test checklist for the author's work machine:
- `scoop install uv just`
- `uv tool install -e .` → `fin agents` works
- `just test` passes
- `just lint` passes
- `just ci` passes (minus `check`/`fmt` which need treefmt — Windows path uses `ruff format --check`)
- `FIN_DATA_DIR=./.fin just run` works (local state)
- Default `DATA_DIR` resolves to `%LOCALAPPDATA%/fin`

#### Touchpoints

| File | Change |
|------|--------|
| `justfile` | Add `windows-shell`, conditionals for `dev`/`fmt`/`check`/`clean`/`diagrams-clean` |
| `src/fin_assist/paths.py` | Platform-aware `_default_data_dir()`, update docstring |
| `tests/test_paths.py` | Add test for Windows default when `sys.platform == "win32"` is mocked |
| `README.md` | Add "Quick start (no Nix)" section |
| `.github/workflows/ci.yml` | Add `test-windows` job on `windows-latest` |
| `AGENTS.md` | Update Local Development Paths table with Windows defaults |
| `docs/configuration.md` | Note Windows path convention |

#### What we explicitly defer

- **Scoop manifest** — requires PyInstaller/Nuitka build pipeline + signing. Not worth it pre-1.0.
- **Windows Terminal detection** — prompt-toolkit handles this; no fin code changes needed.
- **Secrets on Windows** — secretspec doesn't support Windows; document manual env var setup. Future: `/connect` (#124) should work cross-platform.
