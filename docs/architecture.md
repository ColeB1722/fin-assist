...
**Why this matters now.** The platform-stance work retired both the workspace-split forcing function ([#128](https://github.com/ColeB1722/fin-assist/issues/128) deferred indefinitely) and the BFF framing ([#132](https://github.com/ColeB1722/fin-assist/issues/132) rejected on the merits). What replaces them is a sharper justification: **the firewall is what answers the question "if we removed the CLI tomorrow, would external clients see exactly the same hub API the CLI did?"** With the firewall in place the answer is provably yes — the only allowed hub imports from `cli/` are the in-process launcher, and the rest of the surface is what every protocol peer reaches. Without the firewall the answer is "probably, but you'd have to audit."

[#162](https://github.com/ColeB1722/fin-assist/issues/162) (ACP-server first cut) makes this externally testable for the first time. The platform claim — *new inbound consumers are protocol peers, not BFF clients* — is only verified by ACP-server if the CLI is held to the same contract; otherwise we've only confirmed the protocol works for external clients while the internal client used a privileged path. The cost of keeping the rule is essentially zero (the launcher allowlist has not drifted since v0.1.0; `just lint-imports` runs in ~1s); the cost of removing it is making the protocol-peer architecture a convention rather than a rule at exactly the moment it becomes testable.
...
7. **Protocol is the contract** — The hub's externally-visible API *is* its inbound protocol surfaces; no client gets a privileged back-channel. `cli/` does not import from `hub/` (except the in-process launcher path for `fin-assist serve`); `hub/` never imports from `cli/`. This rule is what makes the protocol-peer architecture testable rather than aspirational: the CLI is held to the same contract as Zed-via-ACP or Claude-Desktop-via-MCP, so the dev REPL functions as a genuine dogfood loop for inbound protocol surfaces rather than a BFF in disguise. The Q5 verification argument (see [`decisions.md`](decisions.md#platform-stance)) depends on this. See *Deliverables: Hub vs CLI* above.
